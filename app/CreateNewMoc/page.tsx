...
export default function MOCForm() {
  ...
  const { register, handleSubmit, reset, setValue, control, formState: { errors } } = useForm<MocFormSchema>({
    resolver: zodResolver(mocSchema),
    defaultValues: {
      mocNo: "",
      cwo: "",
  ...
  // Updated API calls
  const fetchMOCs = async (): Promise<ClientMOC[]> => {
  ...
  const createMOC = async (data: MocFormSchema) => {
    const response = await fetch('/api/mocs', {
      method: 'POST',
    ...
    if (!response.ok) throw new Error(result.message || 'Failed to create MOC');
    return result.data;
  };

  const updateMOC = async ({ id, values }: { id: number; values: Partial<MocFormSchema> }) => {
    const response = await fetch(`/api/mocs/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(values)
    });
    const result = await response.json();
    if (!response.ok) throw new Error(result.message || 'Failed to update MOC');
    return result.data;
  };
  ...
  const deleteMOC = async (id: number) => {
    ...
    if (!response.ok) throw new Error('Failed to delete MOC');
    return true;
  };
