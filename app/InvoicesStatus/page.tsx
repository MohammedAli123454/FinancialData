...
export default function InvoicesStatus() {
  const [data, setData] = useState<PartialInvoices[] | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await getGroupedMOCs();
        if (!result.success) {
  ...
  if (!data) return <div className="p-6">No MOC data found</div>;
  ...
  const aggregatedSums = {
    TOTAL_AWARDED_MOCS_AMOUNT_EX_VAT: filteredMOCsForAggregation.reduce(
      (sum, moc) => sum + safeNumber(moc.contractValue),
      0
    ),
    TOTAL_SUBMITTED_INVOICES_W_VAT_NET_RTN: allInvoicesForAggregation.reduce(
      (sum, row) => sum + ((row?.amount ?? 0) + (row?.vat ?? 0) - (row?.retention ?? 0)),
      0
    ),
    TOTAL_PAID_INVOICE_W_VAT_NET_RTN: allInvoicesForAggregation
      .filter(row => row?.invoiceStatus === "PAID")
      .reduce(
        (sum, row) => sum + ((row?.amount ?? 0) + (row?.vat ?? 0) - (row?.retention ?? 0)),
        0
      ),
    INVOICES_U_FINANCE_W_VAT_NET_RTN: allInvoicesForAggregation
      .filter(row => row?.invoiceStatus === "FINANCE")
      .reduce(
        (sum, row) => sum + ((row?.amount ?? 0) + (row?.vat ?? 0) - (row?.retention ?? 0)),
        0
      ),
    INVOICES_U_PMD_W_VAT_NET_RTN: allInvoicesForAggregation
      .filter(row => row?.invoiceStatus === "PMD")
      .reduce(
        (sum, row) => sum + ((row?.amount ?? 0) + (row?.vat ?? 0) - (row?.retention ?? 0)),
        0
      ),
    INVOICES_U_PMT_W_VAT_NET_RTN: allInvoicesForAggregation
  ...
  const paymentPercentage = aggregatedSums.TOTAL_PAID_INVOICE_W_VAT_NET_RTN /
    (aggregatedSums.TOTAL_SUBMITTED_INVOICES_W_VAT_NET_RTN || 1);
